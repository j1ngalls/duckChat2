...
int main(int argc, char *argv[])
{
	...
	//define select to wait for either user input or message from server
	//first use only user input in select. then add socket messages
	...
	//call listen to receive message
	...
	while (cont)
	{
		...
		//for keep alive include a timeout and send a keepalive message if returning
		//from select for timeout
		...
		else
		{
			...
			if (FD_ISSET(STDIN,&fds))
			{
				...
				handle_user_input();
				//for testing - to go out of the loop
				//cont = 0;
				user_data = 1;
			...
			if (!(user_data||server_data))
			{
				...
				int err = send_keep_alive_message();
				if (err)
				{
					printf("Message failed\n");
				}
			...
			//cout << "elapsed time " << (int) t2-t1 <<endl;


			//cout << "Timer seconds " << tv.tv_sec << endl;
			//tv.tv_sec = 60;
			//tv.tv_usec = 0;

			//if socket
			//handle it - received string may have to be a global variable
			//call listening function again
