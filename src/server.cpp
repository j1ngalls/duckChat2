...
int main(int argc, char *argv[])
{
	...
	/*
	map<string,string> current_user_names;
	current_user_names["daya"] = "daya";

	channels["Common"] = current_user_names;

	map<string,string> temp;
	temp = channels["Common"];
	string daya = temp["daya"];
	cout << daya << endl;
	*/
	...
	while(1) //server runs for ever
	{
		...
		else
		{
			...
			else
			{
				...
				//have to be set by any message from user including keep_alive
...
void handle_login_message(void *data, struct sockaddr_in sock)
{
	...
	unsigned short port = sock.sin_port;

	cout << "ip: " << ip << endl;
	cout << "port: " << port << endl;

	unsigned long key = (ip*10000) + port;
	cout << "key: " << key << endl;
	cout << "server: " << username << " logs in" << endl;
	*/

	//testing sending a say message
	/*
	ssize_t bytes;
	void *send_data;
	size_t len;
	...
	strcpy(send_msg.txt_channel,"Common");
	strcpy(send_msg.txt_username,"nsa");
	strcpy(send_msg.txt_text,"Hello out there");
	send_data = &send_msg;

	len = sizeof send_msg;

	cout << username <<endl;
	struct sockaddr_in send_sock = usernames[username];
